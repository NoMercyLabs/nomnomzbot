...
- [ ] the admin pannel for the saas is just show and does not do anything useful, it does not let me change behavior or go into details of things. i can not promote accounts to admin or grant them specific saas permissions. *(Leg 1 SHIPPED backend-side: `PlatformIamController` (§5.4, revised 2026-07-17) — promote user → principal, role catalog + principal lists, assign/revoke roles, effective permissions, deactivate/reactivate with `User.IsPlatformPrincipal` wiring; handoff entry for the UI. Remaining backend legs: tenant ops (suspend/enable channel — `tenant:suspend` key is seeded but unused), AdminHub has zero publishers (dead pipe), `GET admin/health` returns canned "healthy". Feature flags + billing grants were already functional.)*
