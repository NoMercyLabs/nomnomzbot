...
- [ ] the admin pannel for the saas is just show and does not do anything useful, it does not let me change behavior or go into details of things. i can not promote accounts to admin or grant them specific saas permissions. *(Legs 1+2 SHIPPED backend-side: `PlatformIamController` (§5.4) — promote user → principal, role/principal lists, assign/revoke, effective permissions, deactivate/reactivate with `User.IsPlatformPrincipal` wiring; `PlatformAdminController` (stream-admin §3.2/§5) — tenant list/detail, suspend/reinstate (ENFORCED: Gate-1 403 + bot lifecycle drop), audited support access, Plane-C audit search. Handoff entries for both screens. Remaining backend legs: AdminHub has zero publishers (dead pipe), `GET admin/health` returns canned "healthy". Feature flags + billing grants were already functional.)*
