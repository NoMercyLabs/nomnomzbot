...
- ✅ **User-plane topic attribution — SHIPPED 2026-07-11**: `user.update` turned out already per-channel-correct since per-broadcaster tokens (own token + own `user_id`; stays per channel). `user.whisper.message` (the actually-broken one: identical bot-id condition → 409 parade, and dropped-on-ingest with a dedicated bot) now subscribes ONCE as platform tenant `Guid.Empty` (`BotLifecycleService.PlatformEventTypes`; legacy per-channel rows auto-retired), the registry's Channels FK is dropped, and an unresolvable whisper is attributed to the platform sentinel at ingest instead of skipped. Whisper→channel routing still waits for a bot-inbox surface.
