...
- **Spotify completeness (spec-promised, audit 2026-07-04; seam widened 2026-07-05 — flip capability flags as members wire up)** — add: `PUT /me/player/volume` (`SetVolumeAsync` + the `Volume` flag and §3.5 member, see MusicService gate TODO), `POST /me/player/previous` (`Previous` flag), `GET /me/player` full state (sequencer poll), and the §3.10 manage surface (`/me/tracks`, playlist create/update/items, `/me/following`, playlist followers — the `MusicProviderManageApi` front + fail-closed stubs with `// TODO next slice: Spotify §3.10` markers are in place). Also: fold the residual `IMusicRemoteProvider` members into their owning slices (paged playlists → §3.10, play-context → §3.5.2 sequencer) and resolve the DI line-1035 Singleton-registry vs Scoped-provider captive-dependency note when `IMusicProviderRegistry` lands. OWNER-CONFIRM-N/A bucket: queue read, recently-played, top-items, markets, albums/artists/audiobooks/episodes/shows/browse (verify which are Spotify-removed vs merely unused — provider header claims Feb-2026 removals the live reference still lists). Spotify has no webhooks — polling is the only pattern (confirmed).
