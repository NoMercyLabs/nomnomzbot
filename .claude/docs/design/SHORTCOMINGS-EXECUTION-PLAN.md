...
- [ ] **S-OBS-PROTOCOL-GAPS** `GetStats` and `GetVirtualCamStatus` are done (commit `2851691c`).
      `GetSceneTransitionList` and `GetSourceFilterList` are done too (service methods + `ObsTransitionDto`/
      `ObsFilterDto` + `GET scene-transitions`/`GET source-filters?sourceName=...` controller reads +
      fake-transport and controller tests). Still genuinely not implemented server-side in
      `ObsControlService`: studio mode enable/disable — only `TriggerStudioTransitionAsync` exists (needs
      studio mode already on); a real `SetStudioModeEnabled` WRITE is a different shape from every read
      built so far and is a genuine follow-up, not trivial.
...
- **S-OWN08-remaining** — admin pages (SaaS management: users, providers, settings) UX/DX pass. Owner: "the
  ...
  - **Overview/Channels/Users/System/Flags tabs use raw `Row`/`Column`/`Card` list rows with no design-system
    ...
    **CLOSED** — both tabs now carry a server-driven pager (row 26 onward is reachable instead of silently
