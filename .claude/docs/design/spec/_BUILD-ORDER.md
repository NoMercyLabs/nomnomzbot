...
- Twitch Helix client (codegen DTOs, `ITwitchRateLimiter` adaptive limiter, resilience handler); EventSub transport (WS lite / conduit+webhook SaaS) → dispatcher → journal → bus; per-platform chat provider behind `IChatProvider` (Twitch = Helix Send Chat Message; Kick / YouTube / X providers alongside; IRC retired).
