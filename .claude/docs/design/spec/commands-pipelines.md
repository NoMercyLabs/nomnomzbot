...
| `send_whisper` | `{ string TargetUserRef, string Message }` | renders `Message` via `ITemplateEngine` and sends a Twitch whisper (Helix `POST /whispers`) from the bot identity bound to `ctx.BroadcasterId` (broker pattern — no token in config). Gated by `chat:whisper:send`; **rate-limited** per-tenant via the distributed limiter (whisper spam is a ban risk — the limits-baseline applies). `user:manage:whispers` requested progressively. |
