...
| D7 | **Exposed to the Automation API.** `CustomDataReceivedEvent` carries `[Event("Custom.<name>", Public)]` and lands in the unified event catalog (`dev-platform.md` §2 — attribute-driven, no descriptor); its projection is source name, fields, timestamp, so external tools subscribe to custom data just like any other event (`automation-api.md` §4.2, `Custom.*`). |
