...
3. **Fulfill per `PrizeMode`:**
   ...
   - `code_pool` — atomically claim one `available` code from `PrizeCodePoolId` per winner (`→ assigned`, `AssignedWinnerId` set), **whisper** the decrypted code to the winner via `IChatTransport.SendWhisperAsync`; on success `→ delivered`, `WhisperDelivered=true`; on failure leave `assigned`, `WhisperDelivered=false` (broadcaster reveals it via §3.2, D6). Fails `CODE_POOL_EXHAUSTED` if fewer codes than winners (drawn winners without a code are flagged, not silently dropped).
