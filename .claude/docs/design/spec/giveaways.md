...
| D5 | **Four prize modes** (`PrizeMode`, owner-selected all four): **`announce`** (draw + name only); **`currency`** (fixed `PrizeCurrencyAmount` or `PrizeFromPot` = the summed entry costs, credited via economy); **`pipeline`** (run `PrizePipelineId` once per winner — grant/whisper/TTS/webhook, anything the engine does); **`code_pool`** (each winner gets a unique unused code from a `GiveawayCodePool`, delivered by **direct message** on the winner's platform). **Value-out gate (same rule as `economy.md` §3.5 / `live-games.md` D8):** a `code_pool` prize has real-world value, so a `code_pool` giveaway with `EntryCost > 0` (points buy a chance at value-out = gambling) is refused at create/update (`VALUE_OUT_PAID_ENTRY`) unless the channel's `IAgeConsentService` gate is enabled and every entrant has passed it (enforced per entry, `AGE_CONSENT_REQUIRED`); free-entry `code_pool` giveaways carry no gate. `currency`/`announce`/`pipeline` prizes stay fun-money (zero value-out) and are ungated. |
| D6 | **Codes are secrets.** `GiveawayCode.CodeCipher` is **AEAD-encrypted via `IFieldCipher`** ([PII-shred]); the plaintext is **never** returned by a list/read API (masked), only delivered to the winner by direct message through `IPlatformDirectMessageSender` (platform-generic; Twitch = Helix whispers, Kick/YouTube/X = their DM surface where it exists). Where the winner's platform has no DM capability (`CAPABILITY_UNSUPPORTED`) or the send fails (platform restriction), the code stays assigned and **is revealed to the broadcaster in winner history** for manual relay — never lost, never leaked to others. |
...
| **`GiveawayEntry`** | **G.7 (NEW)** `[soft-delete]` `ITenantScoped` | tenant | `Id Guid` PK; `BroadcasterId Guid` FK Index; `GiveawayId Guid` FK→`Giveaway.Id` Index; `ViewerUserId Guid` FK→`Users.Id`; `Provider string(20)` **[VC:enum `Platform`]** + `ProviderUserId string(100)` **[PII-hash]** (platform connection the entry arrived on); `ViewerTwitchUserId string(50)?` **[PII-hash]** (nullable Twitch projection); `TicketCount int` (weighted, D4); `EntryCostLedgerEntryId long?` (if paid); `EnteredAt DateTime`; `CreatedAt/UpdatedAt/DeletedAt`. **Unique** `(GiveawayId, ViewerUserId)`. |
| **`GiveawayWinner`** | **G.8 (NEW)** `[APPEND-ONLY]` `ITenantScoped` | tenant | `Id Guid` PK; `BroadcasterId Guid` FK Index; `GiveawayId Guid` FK Index; `ViewerUserId Guid` FK→`Users.Id`; `Provider string(20)` **[VC:enum `Platform`]** + `ProviderUserId string(100)` **[PII-hash]** (the platform the winner is messaged on); `ViewerTwitchUserId string(50)?` **[PII-hash]** (nullable Twitch projection); `DrawnAt DateTime`; `Status string(20)` **[VC:enum]** (`drawn`\|`claimed`\|`forfeited`\|`redrawn`); `IsRedraw bool`; `AssignedCodeId Guid?` FK→`GiveawayCode.Id`; `FulfillmentLedgerEntryId long?` (currency payout); `WhisperDelivered bool?` (code mode). **Index** `(GiveawayId)`, `(BroadcasterId, DrawnAt)`. |
...
3. **Fulfill per `PrizeMode`:**
   ...
   - `code_pool` — atomically claim one `available` code from `PrizeCodePoolId` per winner (`→ assigned`, `AssignedWinnerId` set), **direct-message** the decrypted code to the winner via `IPlatformDirectMessageSender.SendAsync(provider, providerUserId, text)` (Twitch impl = `ITwitchWhispersApi.SendWhisperAsync`); on success `→ delivered`, `WhisperDelivered=true`; on failure or `CAPABILITY_UNSUPPORTED` (platform has no DM) leave `assigned`, `WhisperDelivered=false` (broadcaster reveals it via §3.2, D6). Fails `CODE_POOL_EXHAUSTED` if fewer codes than winners (drawn winners without a code are flagged, not silently dropped).
