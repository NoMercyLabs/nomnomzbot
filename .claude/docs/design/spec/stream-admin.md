...
> **Convention reconciliation (load-bearing — read once).** The locked schema widens `BroadcasterId` `string`→`Guid` and demotes raw-Twitch-id PKs to indexed `Twitch*Id` attribute columns (schema §1.1). **This spec is written to the locked post-rebuild model: every tenant key is `Guid`** — and the rebuilt code matches it: `PipelineExecutionContext.BroadcasterId` is `Guid`, and the Helix surface is `Guid`-keyed end-to-end (e.g. `ITwitchChannelsApi.ModifyChannelInformationAsync(Guid broadcasterId, …)` resolves `Channels.TwitchChannelId` internally before the Helix call). New service interfaces below take `Guid`; do **not** reintroduce `string` tenant keys.
...
- `GetCurrentMetadataAsync` — reads current `Channels` values (enriched with live viewer count via `ITwitchStreamsApi.GetStreamAsync` when live); no write, no event.
...
| Helix calls (update channel info, category search) | typed `ITwitchHelixClient` sub-clients (`ITwitchChannelsApi`, `ITwitchSearchApi`) over `HttpClient` + Microsoft.Extensions.Http.Resilience (2nd) | stream metadata push |
