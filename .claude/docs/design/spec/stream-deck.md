...
| # | Decision |
|---|---|
| D1 | **The plugin is a client of the Automation API — no new control plane.** Keypress → `POST /automation/v1/invoke` (scope `invoke`); the plugin ships the 19 static music actions **plus one generic "Run pipeline" action** whose picker is populated from `GET /automation/v1/pipelines` (scope `read`) (`streamdeck-plugin.md` P1); live key feedback (title/value/state) is driven client-side from the WS event stream (scope `events`) against the public event catalog. Server-driven "Decks" layouts (Streamer.bot-style) are **not built** — key mapping + feedback live in the plugin, by decision. |
...
| D6 | **Schema: none.** Pairing codes are ephemeral (`ICacheService`); the credential is `AutomationApiToken` (P.17). Adds three endpoints: mint-code (management), redeem (data plane, code-gated), refresh (data plane, token-authed — D8). |
...
`AutomationPairingController`. Three endpoints across the two planes (see `automation-api.md` §4–§5).
