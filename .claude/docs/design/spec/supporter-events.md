...
| D5 | **Optional economy reward.** A per-(source,kind) config can grant currency on a supporter event (reuses `economy.md`); off by default. Alerts ride the system Alert surface (`widgets-overlays.md` §1.2), branching on `Kind`. |
...
Inherit `DomainEventBase`. Published via `IEventBus`; consumed by the trigger source, the system Alert surface, and (opt-in) economy reward.
...
- **Alerts:** the **system Alert surface** (`widgets-overlays.md` §1.2 — the one alert queue across every platform connection) renders supporter events, branching on `Kind`; the event response for `supporter.<kind>` produces the alert like any other event response.
...
### 4.1 Platform-native support events map to the same domain events

Kick subscriptions / gifted subs / Kicks and YouTube memberships / Super Chats are **not** supporter sources (they are platform connections, `twitch-eventsub.md` §3.1 `IEventSource`), and they do **not** get their own triggers: each platform's raw event is translated into the **same `channel.subscribe`-class domain event** Twitch produces, carrying a `Provider` discriminator (`twitch`\|`kick`\|`youtube`). One event response config and the one Alert surface therefore cover every platform; `{{event.provider}}` is available to a response that wants to branch.

| Platform raw event | Domain event (`Provider` set) | In-house name (`widget-sdk.md` §2.1) |
|---|---|---|
| Kick `channel.subscription.new` / `channel.subscription.renewal` | `NewSubscriptionEvent` / `ResubscriptionEvent` (`channel.subscribe`-class) | `viewer.subscribed` |
| Kick `channel.subscription.gifts` | `GiftSubscriptionEvent` | `viewer.gifted` |
| Kick `kicks.gifted` | `CheerEvent` (`Bits` = Kicks) | `bits.cheered` |
| YouTube `newSponsorEvent` / `memberMilestoneChatEvent` | `NewSubscriptionEvent` / `ResubscriptionEvent` (`Tier` = membership level) | `viewer.subscribed` |
| YouTube `membershipGiftingEvent` | `GiftSubscriptionEvent` | `viewer.gifted` |
| YouTube `superChatEvent` / `superStickerEvent` | `CheerEvent` (`Bits` = Super Chat amount in minor units, `Currency` set) | `bits.cheered` |

Third-party money (Ko-fi, Patreon, …) stays `supporter.*` (D1); platform-native money is the platform event. The two never overlap.
...
Seed in `roles-permissions.md`: **`supporters:read`** (`management`, floor Moderator, `Low`), **`supporters:config:write`** (`management`, floor Broadcaster, **`Critical`** — connecting a payout/identity-bearing money source; not permit-grantable). Replaces the old `donations:read`/`donations:config:write`.
