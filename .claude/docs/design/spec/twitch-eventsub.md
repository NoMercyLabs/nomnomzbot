...
### 9.1 Chat read/send boundary (relationship to `IChatTransport`)

Chat **read** (ingest) is EventSub `channel.chat.message` on **both** deployment profiles — this subsystem journals and fans it out via `INotificationDispatcher` like any other notification. There is **no per-channel IRC socket on SaaS** for ingest; IRC `chat:read` is not used (scaling-qos.md §6). Chat **send** is **not** this subsystem — it is the `IChatTransport` adapter (`HelixChatTransport` = Helix `POST /helix/chat/messages` on SaaS, `IrcChatTransport` = TLS IRC on self-host; scaling-qos.md §6), selected on the deployment-profile axis. This subsystem owns inbound events only; outbound chat is the chat-transport subsystem's concern.
