...
| # | Decision |
|---|---|
| D1 | **Transport mirrors `obs-control.md` exactly.** `IVtsTransport` with `DirectVtsTransport` (self-host → `ws://localhost:8001`) and `BridgeVtsTransport` (SaaS → the existing overlay relay). The SaaS bridge **reuses the OBS relay**: the single always-loaded relay overlay page (OBS browser source) proxies localhost WS services generally, so one relay carries both OBS-WS and VTS-WS, with the same single-executor election (one leader per channel) + idempotent `CommandId`. No second bridge mechanism. |
...
Transport mirrors `obs-control.md` (direct self-host / shared overlay relay SaaS) (D1); plugin-token auth stored AEAD (D2); full typed surface + generic `vts_request` (D3); VTS events as `vts_event` triggers, low-volume default-subscribed (D4); schema delta **P.19 `VtsConnection`**, one per channel, opt-in (D5).
