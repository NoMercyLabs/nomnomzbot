...
// Desktop (jvm) animated-image decode via Skia. Compose Multiplatform's desktop target is Skiko-backed, so the
// full `org.jetbrains.skia` codec surface is available; `Codec.readPixels(bitmap, frame)` composites each frame
// (honouring GIF/WebP disposal + blend internally), which we hand to Compose as an `ImageBitmap`.
