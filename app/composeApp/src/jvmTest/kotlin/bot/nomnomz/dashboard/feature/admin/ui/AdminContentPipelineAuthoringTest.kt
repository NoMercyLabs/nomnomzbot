...
/**
 * S-ADMIN-2d: a `Kind = "pipeline"` definition's draft editor must author its `payloadJson` through the SAME
 * tree editor (`ChainEditor`, `bot.nomnomz.dashboard.feature.pipelines.ui`) the tenant-side Pipelines page
 * uses — never a second, parallel implementation (platform-admin.md §2.2). This is proven by asserting the
 * rendered tree editor's OWN controls ("Add step", "Save chain") appear, and that the seeded payload's step
 * round-trips through the real `PipelineStep`/`PipelineNode` model (its configured message text renders),
 * not merely that some editable field exists.
 */
@OptIn(ExperimentalTestApi::class)
class AdminContentPipelineAuthoringTest {
    ...
    private val pipelinePayloadJson: String =
        """{"steps":[{"id":"s1","action":{"type":"send_message","message":"hello from platform"}}]}"""

    @Test
    fun opening_a_pipeline_definition_draft_renders_the_shared_tree_editor_with_the_seeded_step() {
        val definitionId = "def-pipeline-1"
        val versionId = "ver-pipeline-1"
        val definition = PlatformContentDefinition(
            id = definitionId,
            kind = "pipeline",
            key = "raid-start",
            displayName = "Raid Start Flow",
            currentVersion = 1,
        ...
        val version = PlatformContentVersion(
            ...
            version = 1,
            contentHash = "abc123",
            payloadJson = pipelinePayloadJson,
            publishedAt = "2026-09-01T00:00:00Z",
            draftedAt = "2026-09-01T00:00:00Z",
            draftedByPrincipalId = "principal-1",
        )
        val api = FakeContentApiForPipelineTest(
        ...
        val controller = AdminController(
            api = NoopAdminApiForPipelineTest(),
            iamApi = FakeIamApiForPipelineTest(),
            platformAdminApi = NoopPlatformAdminApiForPipelineTest(),
            contentApi = api,
        )
        ...
        runComposeUiTest {
            ...
            // These two controls only exist inside the tenant-side tree editor (`ChainEditor`) — a raw JSON
            // textarea (the pre-existing fallback for kinds with no dedicated authoring UI) would never
            // render them. Their presence is the proof the SAME shared component is mounted here.
            onNodeWithText("Add step").assertExists()
            onNodeWithText("Save chain").assertExists()

            // The seeded step's configured message must have round-tripped through the real
            // `PipelineGraph`/`PipelineStep`/`PipelineNode` decode — not stayed unread inside an opaque
            // JSON string — to prove this is genuine tree authoring, not a read-only shell around the text.
            assertTrue(
                onAllNodesWithText("hello from platform", substring = true).fetchSemanticsNodes().isNotEmpty(),
                "the pipeline payload's seeded step message must render through the shared tree editor",
            )
        }
    }
}

private class FakeContentApiForPipelineTest(
...
private class FakeIamApiForPipelineTest : PlatformIamApi {
...
private class NoopAdminApiForPipelineTest : AdminApi {
...
private class NoopPlatformAdminApiForPipelineTest : PlatformAdminApi {
