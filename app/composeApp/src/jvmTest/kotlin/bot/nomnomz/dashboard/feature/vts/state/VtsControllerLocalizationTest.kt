...
class VtsControllerLocalizationTest {
    ...
    @Test
    fun the_control_action_with_no_active_channel_surfaces_the_localized_dutch_error() = runTest {
        val original: Locale = Locale.getDefault()
        try {
            Locale.setDefault(Locale.forLanguageTag("nl"))
...
// The no-channel path never reaches the VtsApi (load() bails before `refresh()` ever calls it), so every
// member errors loudly if the controller's guard clause ever regresses and starts calling through.
private class UnreachableVtsApi : VtsApi {
