...
### 2026-08-19 — ConfigChanged + RewardChanged hub events never wired on the frontend
- **From:** Stoney_Eagle (via Claude)
- **What:** `HubEvent.kt` (`app/composeApp/src/commonMain/kotlin/bot/nomnomz/dashboard/core/realtime/HubEvent.kt:58-68`) dispatches `ChatMessage`, `StreamStatusChanged`, `StreamInfoChanged`, `AlertTriggered`, `ModAction`, `CommandExecuted`, `RewardRedeemed`, `MusicStateChanged`, `ChannelEvent`, `PermissionChanged`, `ObsBridgeStateChanged` — but has NO case for `"ConfigChanged"` or `"RewardChanged"`. Any SignalR message with either method name is silently dropped by the `when` (falls through with no default logged, confirm before assuming it's silent — but no branch consumes it).
- **Why:** `DashboardNotifier.SendConfigChangedAsync` (`server/src/NomNomzBot.Api/Hubs/DashboardNotifier.cs:190`) is called by `PublishConfigChangedAsync` in 9 backend services — `CommandService`, `TimerManagementService`, `PipelineService`, `InboundWebhookEndpointService`, `OutboundWebhookEndpointService`, `CatalogService` (economy), `ModerationService`, `TtsConfigService`, `WidgetService` — every create/update/delete/toggle on those surfaces pushes a live event that the dashboard never receives. Same class of bug as the 2026-07-18 chat live-push gap, but unfixed on this channel: today, toggling a command/timer/event-response enabled state, editing a pipeline, or changing a webhook in one browser tab/device does NOT live-update another open dashboard session for the same channel — only a manual reload picks up the change. `SendRewardChangedAsync` (`DashboardNotifier.cs:184`) has the identical gap.
- **Where:** add `"ConfigChanged" -> ConfigChanged(json.decodeFromString(first))` and `"RewardChanged" -> RewardChanged(json.decodeFromString(first))` cases to the `HubEvent.kt` `when`, define the corresponding `HubEvent` subtypes + DTOs matching `ConfigChangedDto`/`RewardChangedDto` (`server/src/NomNomzBot.Api/Hubs/Dtos/HubResponseDtos.cs`), and wire a consumer per affected screen (Commands, Timers, Pipelines, Webhooks, Economy/Catalog, Moderation, TTS, Widgets, Rewards) that invalidates/refetches on receipt — mirroring how `StreamInfoChanged`/`PermissionChanged` are already consumed.
- **Done when:** toggling/editing a command, timer, pipeline, webhook, catalog item, moderation rule, TTS config, widget, or reward in one open dashboard session live-updates a second open session for the same channel without a reload.
