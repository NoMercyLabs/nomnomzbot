...
{
    // This page's whole logic is one inline <script> with no per-request content, so a per-response nonce
    // (mirroring OverlayHostController's ContentSecurityPolicy/ConfigScript pattern) is the fix: the browser
    // refuses ANY inline script that doesn't carry this exact nonce, so a relayed payload could never inject
    // and run its own <script> even though this page never markup-injects relayed text in the first place.
    ...
    public IActionResult Get()
    {
        ...
        return Content(Html(nonce), "text/html");
    }
}
