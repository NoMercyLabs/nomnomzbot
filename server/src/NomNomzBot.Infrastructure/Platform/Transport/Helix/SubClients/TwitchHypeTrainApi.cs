...
/// <summary>
/// The Helix "Hype Train" sub-client (twitch-helix.md §3.2). Pure Helix I/O: it resolves the tenant
...
/// </summary>
public sealed class TwitchHypeTrainApi(
...
) : ITwitchHypeTrainApi
{
    ...
    )
    {
        ...
        if (scope.IsFailure)
            return scope.WithValue<TwitchHypeTrainStatus>(default!);

        Result<string> channel = await ResolveAsync(broadcasterId, ct);
        if (channel.IsFailure)
            return channel.WithValue<TwitchHypeTrainStatus>(default!);
        ...
        return await transport.GetSingleAsync<TwitchHypeTrainStatus>(request, ct);
    }
