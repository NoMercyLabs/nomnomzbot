...
/// <summary>
/// The Helix "Raids" sub-client (twitch-helix.md §3.2). Pure Helix I/O: it resolves the tenant
...
/// </summary>
public sealed class TwitchRaidsApi(
...
) : ITwitchRaidsApi
{
    ...
    {
        Result scope = await RequireScopeAsync(broadcasterId, TwitchScopes.ChannelManageRaids, ct);
        if (scope.IsFailure)
            return scope.WithValue<TwitchRaid>(default!);

        Result<string> channel = await ResolveAsync(broadcasterId, ct);
        if (channel.IsFailure)
            return channel.WithValue<TwitchRaid>(default!);
        ...
        return await transport.SendWithResultAsync<TwitchRaid>(request, ct);
    }

    public async Task<Result> CancelRaidAsync(Guid broadcasterId, CancellationToken ct = default)
    {
        Result scope = await RequireScopeAsync(broadcasterId, TwitchScopes.ChannelManageRaids, ct);
        if (scope.IsFailure)
            return scope;
