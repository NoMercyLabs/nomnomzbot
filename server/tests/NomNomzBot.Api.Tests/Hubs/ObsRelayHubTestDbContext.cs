...
/// <summary>
/// A focused <see cref="IApplicationDbContext"/> over only <see cref="ObsConnection"/> — on the EF Core
/// SQLite harness — for <see cref="NomNomzBot.Api.Hubs.OBSRelayHub"/> tests, which resolve the channel's
/// OBS connection row by <c>BridgeToken</c> on connect. Everything else throws, since those tests never
/// reach it. Mirrors the same "declare every DbSet, auto-ignore the unmapped ones" shape as
/// <c>Hubs/WidgetTestDbContext.cs</c>, just with <see cref="ObsConnection"/> as the one live entity.
/// </summary>
internal sealed class ObsRelayHubTestDbContext : DbContext, IApplicationDbContext
{
    ...
    private ObsRelayHubTestDbContext(
        DbContextOptions<ObsRelayHubTestDbContext> options,
        SqliteConnection connection
    )
        : base(options) => _connection = connection;

    public static ObsRelayHubTestDbContext New()
    {
        SqliteConnection connection = new("Data Source=:memory:");
        connection.Open();
        ObsRelayHubTestDbContext db = new(
            new DbContextOptionsBuilder<ObsRelayHubTestDbContext>().UseSqlite(connection).Options,
            connection
        );
    ...
    public DbSet<ObsConnection> ObsConnections => Set<ObsConnection>();

    protected override void OnModelCreating(ModelBuilder b)
    {
        b.Entity<ObsConnection>(e => e.HasKey(c => c.Id));
    ...
    private static readonly HashSet<Type> Mapped = [typeof(ObsConnection)];
