...
/// <summary>
/// A focused <see cref="IApplicationDbContext"/> over <see cref="Channel"/> and
/// <see cref="ChannelBuiltinCommand"/> — on a real relational SQLite database, WITH the
/// <c>ChannelBuiltinCommand.Channel</c> navigation left mapped so EF creates the real FK
/// (<c>BroadcasterId</c> → <c>Channels.Id</c>, cascade, required) that S-SEED-GUIDCASE reproduces.
/// <c>CommandsTestDbContext</c> ignores that navigation and so cannot reproduce the FK crash.
/// Everything else throws, since <c>DefaultCommandsSeederGuidCasingTests</c> never reaches it.
/// </summary>
internal sealed class DefaultCommandsSeederGuidCasingTestDbContext
    : DbContext,
        IApplicationDbContext
{
    private readonly SqliteConnection _connection;

    private DefaultCommandsSeederGuidCasingTestDbContext(
        DbContextOptions<DefaultCommandsSeederGuidCasingTestDbContext> options,
        SqliteConnection connection
    )
        : base(options) => _connection = connection;

    public static DefaultCommandsSeederGuidCasingTestDbContext New()
    {
        SqliteConnection connection = new("Data Source=:memory:");
        connection.Open();
        DefaultCommandsSeederGuidCasingTestDbContext db = new(
            new DbContextOptionsBuilder<DefaultCommandsSeederGuidCasingTestDbContext>()
    ...
    protected override void OnModelCreating(ModelBuilder b)
    {
        ...
        {
            e.HasKey(c => c.Id);
            // Intentionally NOT ignored — the real Channel FK is the whole point of this fixture.
        });
