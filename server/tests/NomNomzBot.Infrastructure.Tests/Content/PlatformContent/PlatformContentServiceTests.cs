...
public sealed class PlatformContentServiceTests : IAsyncDisposable
{
    ...
    private async Task<(PlatformContentDefinition Definition, PlatformContentVersion V1)> SeedPublishedDefinitionAsync(
        string key = "sr"
    )
    {
    ...
    public async Task PreviewPublish_ReturnsRealCounts_AndWritesNothing()
    {
        ...
        List<ChannelBuiltinCommand> rows = await _db.ChannelBuiltinCommands.AsNoTracking().ToListAsync();
    ...
    public async Task Publish_WritesAuditRecordWithAffectedCountAndJobId()
    {
        ...
        IamAuditLog auditRow = await _db.IamAuditLogs.AsNoTracking().SingleAsync(a =>
            a.PublishJobId == jobId
        );
    ...
    public async Task Publish_WithStalePreviewCount_FailsClosedAsPreviewStale()
    {
        ...
        Result<PlatformContentPublishJobDto> publishResult = await sut.PublishAsync(
            ...
            definition.Id,
            v2.Id,
            new PublishContentRequest(PlatformContentPublishModes.UpdateInPlaceWhereUntouched, null, 5)
        );
