...
internal sealed class ActionRequiredInboxServiceTestDbContext : DbContext, IApplicationDbContext
{
    ...
    private ActionRequiredInboxServiceTestDbContext(
        DbContextOptions<ActionRequiredInboxServiceTestDbContext> options,
        SqliteConnection connection
    )
        : base(options) => _connection = connection;

    public static ActionRequiredInboxServiceTestDbContext New()
    {
        SqliteConnection connection = new("Data Source=:memory:");
        connection.Open();
        DbContextOptionsBuilder<ActionRequiredInboxServiceTestDbContext> optionsBuilder =
            new DbContextOptionsBuilder<ActionRequiredInboxServiceTestDbContext>().UseSqlite(
                connection
            );
        ActionRequiredInboxServiceTestDbContext db = new(optionsBuilder.Options, connection);
