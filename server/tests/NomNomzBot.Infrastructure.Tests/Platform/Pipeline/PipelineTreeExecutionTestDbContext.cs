...
/// </summary>
internal sealed class PipelineTreeExecutionTestDbContext : DbContext, IApplicationDbContext
{
    private readonly Microsoft.Data.Sqlite.SqliteConnection _connection;

    private PipelineTreeExecutionTestDbContext(
        DbContextOptions<PipelineTreeExecutionTestDbContext> options,
        Microsoft.Data.Sqlite.SqliteConnection connection
    )
        : base(options) => _connection = connection;

    /// <summary>
    /// Real SQLite (not EF InMemory) — <see cref="PipelineEngine"/>'s retention sweep
    /// (<c>PurgeOldExecutionsAsync</c>) uses <c>ExecuteDeleteAsync</c>, which the InMemory
    /// provider does not support.
    /// </summary>
    public static PipelineTreeExecutionTestDbContext New()
    {
        Microsoft.Data.Sqlite.SqliteConnection connection = new("DataSource=:memory:");
        connection.Open();
        PipelineTreeExecutionTestDbContext ctx = new(
            new DbContextOptionsBuilder<PipelineTreeExecutionTestDbContext>()
    ...
    protected override void OnModelCreating(ModelBuilder b)
    {
        ...
        b.Entity<PipelineStep>(e =>
        {
            ...
            e.HasMany(s => s.Conditions).WithOne().HasForeignKey(c => c.PipelineStepId);
        });
