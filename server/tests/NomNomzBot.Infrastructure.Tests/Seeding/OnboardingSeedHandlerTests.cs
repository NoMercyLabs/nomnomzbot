...
public sealed class OnboardingSeedHandlerTests
{
    ...
    /// <summary>Config pinning which single bot identity this deployment may auto-grant.</summary>
    private static IConfiguration SaasBotConfig(string? userId = null, string? username = null)
    {
        Dictionary<string, string?> values = [];
        if (userId is not null)
            values["Twitch:BotUserId"] = userId;
        if (username is not null)
            values["Twitch:BotUsername"] = username;
        return new ConfigurationBuilder().AddInMemoryCollection(values).Build();
    }

    [Fact]
    public async Task BotJoin_handler_refuses_to_grant_moderator_to_an_account_that_is_not_the_configured_saas_bot()
    {
        // 2026-09-04: a personal Twitch account sat in the shared-bot slot for ninety minutes and the grant
        // path handed it moderator on someone else's channel. Granting a moderator role is the
        // broadcaster's call, so the ONE identity this deployment may auto-grant is pinned in config and
        // everything else is refused — even while it is genuinely registered as the shared bot.
        AuthDbContext db = AuthTestBuilder.NewContext();
        db.BotAccounts.Add(SharedBot());
        await db.SaveChangesAsync();
    ...
    [Fact]
    public async Task BotJoin_handler_grants_nothing_when_no_saas_bot_identity_is_configured()
    {
        // A deployment that never named its bot has not consented to hand anyone moderator. Defaulting to
        // "grant whatever happens to be registered" is precisely the behaviour that caused the incident.
        AuthDbContext db = AuthTestBuilder.NewContext();
        db.BotAccounts.Add(SharedBot());
        await db.SaveChangesAsync();
    ...
    [Fact]
    public async Task BotJoin_handler_still_grants_when_only_the_username_is_pinned()
    {
        // Deployments that pinned a name rather than an id keep working. Matched case-insensitively because
        // Twitch display names and logins differ in case.
        AuthDbContext db = AuthTestBuilder.NewContext();
        db.BotAccounts.Add(SharedBot());
        await db.SaveChangesAsync();
