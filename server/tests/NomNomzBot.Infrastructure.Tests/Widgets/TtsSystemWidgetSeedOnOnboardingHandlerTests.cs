...
/// (and on first use if missing)" — before this handler, <c>tts_caption</c> only ever appeared lazily, the
/// first time a streamer opened the TTS page (<c>EnsureSystemWidgetAsync</c>'s "on first use" leg). This
/// handler is the "at channel creation" leg: it hangs off the same <c>ChannelOnboardedEvent</c> fan-out every
/// other onboarding seed job uses, so a brand-new channel already has its TTS system surface the instant
/// onboarding completes, with no dashboard visit required.
/// </summary>
public sealed class TtsSystemWidgetSeedOnOnboardingHandlerTests
{
    ...
    [Fact]
    public async Task Handle_provisions_the_tts_caption_system_widget_for_the_onboarded_channel()
    {
        ...
        TtsSystemWidgetSeedOnOnboardingHandler handler = new(
            widgets,
            NullLogger<TtsSystemWidgetSeedOnOnboardingHandler>.Instance
        );
    ...
    [Fact]
    public async Task Handle_swallows_a_failure_from_EnsureSystemWidgetAsync_never_throws()
    {
        IWidgetService widgets = Substitute.For<IWidgetService>();
        Guid broadcasterId = Guid.CreateVersion7();
        widgets
