...
# -----------------------------------------------------------------------------
#
# reinstall-local.ps1 — dev-loop helper: build the OBS plugin, install it into this machine's
# Elgato Stream Deck plugins folder, restart the app, and confirm a clean reconnect from
# its own log (no crash). Run from tools/streamdeck/obs/.
...
$src = Join-Path $PSScriptRoot "..\bot.nomnomzbot.streamdeck.obs.sdPlugin" | Resolve-Path
$dst = "$env:APPDATA\Elgato\StreamDeck\Plugins\bot.nomnomzbot.streamdeck.obs.sdPlugin"
...
for ($i = 0; $i -lt 15 -and -not $connected; $i++) {
    Start-Sleep -Seconds 2
    if (Test-Path $log) {
        $connected = Select-String -Path $log -Pattern "\[bot\.nomnomzbot\.streamdeck\.obs\] Plugin connected" |
