...
// src/ instead, one level above tools/streamdeck/obs/<sdPlugin>/imgs/actions/. Both are real,
...
const CANDIDATE_DIRS = [
  ...
  join(here, "..", "bot.nomnomzbot.streamdeck.obs.sdPlugin", "imgs", "actions"),
];
const ACTIONS_ICON_DIR = CANDIDATE_DIRS.find((dir) => existsSync(dir)) ?? CANDIDATE_DIRS[0]!;

const cache = new Map<string, string>();

/** Raw SVG markup for a recolored (white) action icon, by its manifest base name (e.g. "scene"). */
